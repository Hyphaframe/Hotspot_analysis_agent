...
# -*- coding: utf-8 -*-
"""
Streamlit Web 界面 - 社交媒体热点分析智能体系统
集成热榜爬取功能和舆情分析功能
"""
...
def main() -> None:
    ...
    st.title("📱 社交媒体热点分析智能体系统")
    st.markdown("基于 LangGraph 的社交媒体热点 AI 代理")
    ...
    try:
        ...
        default_config = None
        has_config_file = False
        st.sidebar.warning("⚠️ 未找到配置文件，请手动输入 API 密钥")
    ...
    if enable_hot_topics:
        ...
        if st.session_state.hot_topics:
            # 平台筛选
            platforms = list({topic.platform for topic in st.session_state.hot_topics})
            selected_platform = st.selectbox("筛选平台", ["全部"] + platforms)
                ...
                with st.container():
                    ...
                    with col2:
                        if st.button(
                            ...
                            st.session_state.selected_topic = topic.title
                            st.session_state.selected_hot_topic = {
                                "id": topic.id,
                                "title": topic.title,
    ...
    if "selected_topic" in st.session_state:
        st.info(
            f"🎯 已选择热点话题：**{st.session_state.selected_topic}** "
            f"({st.session_state.get('selected_platform', '')})"
        )
        query = st.text_area(
            "分析主题",
            value=st.session_state.selected_topic,
    ...
    if start_research:
        ...
        try:
            ...
            final_report = None
            # 获取热点信息
            hot_topic_info = st.session_state.get("selected_hot_topic", None)

            for progress_data in agent.research(
                query, save_report=save_report, hot_topic_info=hot_topic_info
            ):
                if progress_data["node"] == "completed":
                    final_report = progress_data["report"]
                    status_placeholder.success("✅ 分析完成！")
    ...
    if "selected_topic" in st.session_state:
        if st.button("🗑️ 清除选择的热点话题"):
            ...
            if "selected_platform" in st.session_state:
                del st.session_state.selected_platform
            if "selected_hot_topic" in st.session_state:
                del st.session_state.selected_hot_topic
            st.rerun()
