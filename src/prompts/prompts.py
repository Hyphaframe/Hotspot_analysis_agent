...
确保段落排序符合分析与决策的逻辑流程；选择4-7个最重要的，最多保留7个段落。  
...
# 每个段落第一次搜索的系统提示词（社媒搜索）
SYSTEM_PROMPT_FIRST_SEARCH = f"""
你是一位社交媒体搜索与情报检索专家。

你将获得一个报告段落的标题与预期内容（例如“核心话题与关键词”或“情感与舆情倾向”），需要针对该段落生成针对社媒的精准搜索查询。  

热点话题信息（如果有）：  
- 平台：{{hot_topic_info.get('platform', '未知')}}  
- 热度值：{{hot_topic_info.get('hot_value', 0)}}  
- 排名：{{hot_topic_info.get('rank', 0)}}  
- 时间戳：{{hot_topic_info.get('timestamp', '')}}  

请优先考虑下列搜索策略：  
- 指定平台（如baidu、Weibo、Douyin/TikTok、Zhihu）与地域过滤（如国家/省份）  
...
你是一位资深社媒分析师。你将获得段落标题、热点话题信息，预期内容以及该段落当前的最新状态，请反思当前段落是否遗漏了重要的社媒维度或证据，并提出新的搜索查询以补强分析。  
